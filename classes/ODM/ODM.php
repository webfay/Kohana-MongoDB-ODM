...
class Kohana_ODM extends Model {
	...
	*/
	 ...
	 * Find documents in the collection
	 *
	 * @param string $index_field key of value in array
	 * @return ODM_Collection
	 */
	public function find_all($index_field = null)
	{
		$cursor = $this->_cursor_functions($this->_find());
		$result = array();

		if($index_field && !isset($this->_schema[$index_field]))
			throw new Kohana_Exception('Key '. $index_filed. ' is not found in schema');

		if(!$index_field)
		{
			foreach ($cursor as $document)
			{
				$document = ODM::factory($this->_object_name)->load($document);
				$result[] = $document;
			}
